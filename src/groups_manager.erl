...
init_state(Id) ->
    ...
    ?LOG_INFO("Directory: ~p.", [CurrentDirectory]),
    ?LOG_INFO("Groups file: ~p.", [GroupsFile]),
...
get_mypath(MyId, Paths) ->
    case dict:find(MyId, Paths) of
        {ok, Value} ->
            {ok, Value};
        error ->
            ?LOG_ERROR("Wrong deafult tree file, returning []"),
            {ok, []}
    end.
...
new_treefile(File) ->
    ...
    case file:read_line(TreeFile) of
        eof ->
            ?LOG_ERROR("Empty file: ~p", [File]),
            file:close(TreeFile),
            {error, empty_file};
        {error, Reason} ->
            ?LOG_ERROR("Problem reading ~p file, reason: ~p", [File, Reason]),
            file:close(TreeFile),
            {error, Reason};
        {ok, Line} ->
...
replication_groups_from_file(Device, Table)->
    case file:read_line(Device) of
        eof ->
            ok;
        {error, Reason} ->
            ?LOG_ERROR("Problem reading ~p file, reason: ~p", [?GROUPSFILE, Reason]),
            {error, Reason};
        {ok, "\n"} ->
            replication_groups_from_file(Device, Table);
        {ok, Line} ->
...
tree_from_file(Device, Counter, LeavesLeft, Tree0, Path0)->
    case file:read_line(Device) of
        eof ->
            {Tree0, Path0};
        {error, Reason} ->
            ?LOG_ERROR("Problem reading ~p file, reason: ~p", [?TREEFILE, Reason]),
            {Tree0, Path0};
        {ok, Line} ->
